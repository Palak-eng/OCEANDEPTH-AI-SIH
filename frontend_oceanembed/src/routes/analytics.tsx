...
import { fetchMetrics, type BackendMetrics } from "@/lib/backend";
...
const PIPELINE = [
  { step: "01", title: "Ingest", detail: "SST, SSS, SLA/SSH, surface currents (U,V), winds (U,V)" },
  {
    step: "02",
    title: "Harmonise",
    detail: "Regrid to 0.25° × 0.25°, daily compositing, gap filling",
  },
  ...
  { step: "04", title: "Reconstruct", detail: "Attention decoder to 15 standard depth levels" },
  {
    step: "05",
    title: "Validate",
    detail: "Independent gridded ARGO profiles, RMSE / corr / bias",
  },
];
...
function AnalyticsPage() {
  ...
  const live = Boolean(metrics?.available && metrics.metrics);

  const overallCards = live
    ? [
        { label: "RMSE", value: `${metrics!.metrics!.rmse_c.toFixed(2)} °C` },
        { label: "Correlation", value: metrics!.metrics!.correlation.toFixed(2) },
        { label: "Bias", value: `${metrics!.metrics!.bias_c.toFixed(2)} °C` },
      ]
    : SKILL_METRICS;

  const fallbackRows = data.levels.map((l) => ({
    depth: l.depth,
  ...
  const rows =
    live && (metrics?.per_depth.length ?? 0) > 0
      ? metrics!.per_depth.map((entry) => ({
          depth: entry.depth_m,
          rmse: entry.rmse_c.toFixed(2),
          bias: entry.bias_c.toFixed(2),
          corr: entry.correlation.toFixed(2),
          extra: entry.n ?? 0,
        }))
      : fallbackRows;
      ...
      <div className="flex flex-col gap-4">
          ...
          {overallCards.map((m) => (
            <div key={m.label} className="panel-surface px-5 py-4">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                {live && metrics?.model_name
                  ? `Validated against independent gridded ARGO · ${metrics.model_name}`
        ...
        <Panel title="Depth-wise skill scores" bodyClassName="p-0">
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.depth} className="border-t border-border hover:bg-secondary/40">
                    ...
                    <td className="px-4 py-2 text-right text-muted-foreground">{r.extra}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <p className="px-4 py-2 text-xs text-muted-foreground">
            {live
              ? `Source: ${metrics?.validation?.dataset ?? "Gridded ARGO"} · ${metrics?.validation?.period ?? ""} · ${metrics?.validation?.region ?? ""}`
              : "Simulated demo metrics — will be replaced by real ARGO skill scores once model/metrics.json is delivered."}
          </p>
        </Panel>
      </div>
    </AppShell>
